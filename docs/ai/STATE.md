...
**Status**: COMPLETE — env review rules updated and validation passed
...
14. **Review rule cleanup**: Updated Bugbot, agent, and Cloud Agent guidance to prohibit real env files and secret values while allowing `.env.example` only when every value is an empty placeholder.
