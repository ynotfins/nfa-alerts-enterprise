...
- Flag secret leaks: `.env`, `.env.local`, `.env.production.local`, `.env.*.local`, service account JSON, private keys, webhook tokens, API keys, screenshots, or logs containing credentials.
- Allow `.env.example` only when every value is an empty placeholder. `.env.example` must never contain real secrets.
