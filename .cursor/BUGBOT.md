...
- Secrets exposure: committed `.env*` files except `.env.example`, service account JSON, tokens in logs, credentials in docs/screenshots, and code that prints secret values. The repo `.gitignore` intentionally ignores `.env` and `.env.*`; `.env.example` is the only allowed env-file exception.
...
- Do not flag `.env.example` only when every assignment is exactly `KEY=` with nothing after `=`. Flag any non-empty `.env.example` value, including `your-key-here`, `example`, `changeme`, fake tokens, or real tokens.
