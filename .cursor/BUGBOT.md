...
- Secrets exposure: committed `.env`, `.env.local`, `.env.production.local`, `.env.*.local`, service account JSON, tokens in logs, credentials in docs/screenshots, and code that prints secret values.
...
- Do not flag `.env.example` when every value is empty placeholder text; flag it if any real value, token, API key, private key, or credential appears after `=`.
